    ...
    <div>
      <div className="flex items-start justify-between mb-4">
        <p className="text-sm text-zinc-500 max-w-lg">
          Aliases map human-readable names to target containers in your integrations. Reference them in{' '}
          <code className="font-mono text-xs bg-zinc-100 dark:bg-zinc-800 px-1 py-0.5 rounded">.mdspecmap</code>{' '}
          as the <code className="font-mono text-xs bg-zinc-100 dark:bg-zinc-800 px-1 py-0.5 rounded">parent</code> field.
        </p>
        ...
        <form onSubmit={createAlias} className="rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-5 mb-4 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <Field
              ...
              hint="Lowercase, hyphens allowed. Used in .mdspecmap."
            />
            <div>
      ...
      {aliases.length === 0 && connectedIntegrations.length > 0 && !showForm && (
        <p className="text-sm text-zinc-400 py-4">No aliases yet. Create one to reference it as <code className="font-mono text-xs">parent</code> in your .mdspecmap.</p>
      )}
    </div>
  )
}
